...
def remove_the_tracking(parsed_url):
    ...
    if "youtu" in parsed_url.netloc:
        ...
        filtered_params = {
            key: value
            for key, value in query_params.items() if key == 'v'
        }
...
async def get_dl(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(command_parts) < 2:
        await update.effective_message.reply_text(
            "Please provide a URL after the /get command.")
        return
    ...
    if (bool(parsed_url.scheme) and bool(parsed_url.netloc)):
        ...
        if command == "/get":
            ...
            await update.message.reply_text(
                f'dl link: {dl_domain}v/{encoded_url}')
        else:
...
async def help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        f'Send me a url to remove tracking parameters. Tested on YT, IG, TikTik, Reddit, eBay, and more'
    )


async def removeTracking(update: Update,
